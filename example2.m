...
% >>> Fitting I and Anisotropy Estimation <<<
...
b = [gexpmax,maxdist*1/3,gexpmax/100]; % s2,xi & c0
b_lb = [eps,eps,eps]; b_ub = [gexpmax*1.5,maxdist,gexpmax/5]; %lower and upper limits
bsp = [1000,maxdist*1/3,gexpmax/100]; % eta0, xi, c0
bsp_lb = [eps,eps,eps]; bsp_ub = [inf,maxdist,gexpmax/5]; %lower and upper limits
...
R(n_models,1)=0;phi(n_models,1)=0;xi1(n_models,1)=0;xi2(n_models,1)=0;%matrices preallocation
er1(n_models,1)=0;er2(n_models,1)=0; %matrices preallocation
objmod = 'NWEr_m';N_ms = 20;flag = 1;
% nrsampl = 10; samplpc = 50;
for i=1:n_models
    ...
    model.paramsub = model_par_ub{i,1};
     ...
     [R(i,1),phi(i,1),xi1(i,1),xi2(i,1),er1(i,1),er2(i,1)] =...
         aniso_dvf(x,y,rf,model,objmod,N_ms,ncpc,nrbins,phistep,phitol,flag);
%     [R(i,1),phi(i,1),xi1(i,1),xi2(i,1),er1(i,1),er2(i,1)] =...
%         dirvar_ccv_s(x,y,rf,model,objmod,N_ms,ncpc,nrsampl,samplpc,nrbins,phistep,phitol,flag);

end

% >>> Fitting II and Parameters of Correlation Estimation (s2,c0,v or eta1) <<<

% Initial Values and Limits for optimization
b{n_models,1}=[];b_lb{n_models,1}=[];b_ub{n_models,1}=[];
for i = 1:n_models
    b{i,1} = [gexpmax,xi1(i,1),R(i,1),phi(i,1),gexpmax/100];
    b_lb{i,1} = [eps,xi1(i,1),R(i,1),phi(i,1),eps];
    b_ub{i,1} = [inf,xi1(i,1),R(i,1),phi(i,1),gexpmax/5];
end

% Summary cells
model_par0 = {[b{1,1},1.1];b{2,1};b{3,1};[b{4,1},1.5];[b{5,1},1]}; %initial parameters values
model_par_lb = {[b_lb{1,1},eps];b_lb{2,1};b_lb{3,1};[b_lb{4,1},0.3];[b_lb{5,1},-2+eps]};%lower bounds
model_par_ub = {[b_ub{1,1},2];b_ub{2,1};b_ub{3,1};[b_ub{4,1},3.5];[b_ub{5,1},inf]}; %upper bounds

% Estimation of Parameters (s2,c0,v or eta1)
bmodel{n_models,1}=[]; fval(n_models,1)=0;tit{n_models,1}=[];
iso = 00;objmod = 'NWEr_m';N_ms = 20;flag = 1;
for i=1:n_models
    model.function = models{i,1};
    model.params0 = model_par0{i,1};
    model.paramslb = model_par_lb{i,1};
...
for i=1:n_models
    ...
    cv_realZ(i,1) = isreal(cv_matr{i,1}.Z_ibt1);  
...
for i=1:N_kr_mod
    ...
    realCI(i,1) = isreal(CI1{i,1}.uncer); 
end
...
for i=1:N_kr_mod
    ...
    kr_realZ(i,1) = isreal(Z_ibt1{i,1});  
